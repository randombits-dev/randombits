...
const fixedClases = ['sticky!', 'top-24!'];
      ...
      <nav aria-label="Table of Contents">
        {
          relatedPages.sort((a, b) => a.data.order - b.data.order).map(article => {
            ...
            if (showLink) {
              return <div>
                <div class="font-bold" aria-selected={!selected()}>{article.data.toc || article.data.title}</div>
                <div class="rb-article__sub-nav">
                  {
                    headings.map((heading, i) => {
                      const isSelected = selected() === heading.slug;
                      return <div><a class="" href={'#' + heading.slug} aria-selected={isSelected}
                                     onclick={() => handleClickHeading(heading)}>{heading.text}</a></div>;
                    })
                  }
                </div>
              </div>;
      ...
      <nav aria-label="Article Sections">
        {
          headings.map(heading => {
            ...
            return <div><a href={'#' + heading.slug} aria-selected={isSelected}
                           onclick={() => handleClickHeading(heading)}>{heading.text}</a></div>;
          })
        }
      </nav>
